...
public class MapPractice2 {
    ...
    public static void main(String[] args) {
        ...
        // names of employees who makes the salary between 120k - 150k
        map.entrySet()
                .stream()
                .filter(p -> (p.getValue() >= 120000 && p.getValue() <= 150000))
                .forEach(p -> System.out.println(p.getKey()));
        ...
        map.entrySet()
                .stream()
                .filter(p -> p.getValue() < 120000)
                .forEach(p -> p.setValue(p.getValue() + 10000));
...
/*
1. Given the following map that contains the employee name and their salary:
     Map<String, Integer> map = new LinkedHashMap<>();
        map.put("John", 123000);
        map.put("Antony", 100000);
        map.put("Jimmy", 115000);
